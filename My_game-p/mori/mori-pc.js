...
interactiveElements.forEach((el) => {
  el.addEventListener("mouseenter", () => {
    circles.forEach((c) => {
      c.element.style.borderColor = "rgb(64, 109, 255)";
      c.element.style.backgroundColor = "rgba(26, 71, 139, 0.2)";
    });
  });
  el.addEventListener("mouseleave", () => {
    circles.forEach((c) => {
      c.element.style.borderColor = "rgb(113, 215, 247)";
      c.element.style.backgroundColor = "rgba(113, 215, 247, 0.2)";
    });
  });
});
...
function createDroplet(x, y) {
  ...
  // 랜덤 파란색 계열 설정 (Hue 180 ~ 240: 청록색에서 파란색 사이)
